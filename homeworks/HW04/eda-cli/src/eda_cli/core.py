...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col in summary.columns:
        ...
        if col.is_numeric:
            count_of_numeric += 1
            col_percent_of_zero = col.example_values.count(0) / len(col.example_values)
            many_zero_values += 1 if col_percent_of_zero > 0.4 else 0

    flags["has_constant_columns"] = has_constant_columns
    if count_of_numeric == 0:
        flags["has_many_zero_values"] = False
    else:
        flags["has_many_zero_values"] = True if many_zero_values / count_of_numeric > 0.4 else False


    # Простейший «скор» качества
    score = 1.0
    score -= 0.1 if has_constant_columns else 0.0
    score -= 0.1 if flags["has_many_zero_values"] else 0.0
